...
$url = new moodle_url('/repository/files.php');

$baseurl = new moodle_url('/repository/files.php');
...
$url = new moodle_url($CFG->httpswwwroot."/repository/files.php", $params);
$PAGE->set_url('/repository/files.php', $params);
...
case 'sign':
    ...
    if ($repo->check_login()) {
        ...
        if (!empty($list['upload'])) {
            ...
            if (!empty($list['page'])) {
                // TODO: need a better solution
                $pagingurl = new moodle_url("$CFG->httpswwwroot/repository/files.php?action=list&itemid=$itemid&ctx_id=$contextid&repo_id=$repo_id&course=$courseid");
