...
class CMISRepositoryWrapper
{
    ...
    {
        $doc = new DOMDocument();
        $doc->loadXML($xmldata, LIBXML_NOWARNING);// Suppress xml warnings
    ...
    static function extractObjectFromNode($xmlnode)
    {
        ...
        foreach ($prop_nodes as $pn)
        {
            if ($pn->attributes)
            {
                if ($pn->attributes->getNamedItem("propertyDefinitionId") && $pn->getElementsByTagName("value")->item(0)) {
    ...
    static function extractObjectEntry($xmldata)
    {
        //Assumes only one workspace for now
        $doc = new DOMDocument();
        $doc->loadXML($xmldata);
        return CMISRepositoryWrapper :: extractObjectEntryFromNode($doc);
    }
    static function extractObjectEntryFromNode($xmlnode)
    {
        // Process a feed and extract the objects
        //   Does not handle hierarchy
        //   Provides two arrays
...
// Constant for valid xml response
define("VALID_XML_RESPONSE", "399");
...
class CMISService extends CMISRepositoryWrapper
{
    ...
    { //yes
        ...
        $ret = $this->doGet($myURL);
//print_object($this->extractObjectFeed($ret->body));
        //need to return false and calling function can handle
        if ($ret->code <= VALID_XML_RESPONSE) {
