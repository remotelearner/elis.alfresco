...
define('ALFRESCO_ROLE_COORDINATOR',  'Coordinator');   // The coordinator gets all permissions and permission groups defined
define('ALFRESCO_ROLE_COLLABORATOR', 'Collaborator');  // Combines Editor and Contributor permission groups
define('ALFRESCO_ROLE_CONTRIBUTOR',  'Contributor');   // Includes the Consumer permission group and adds AddChildren and CheckOut.
define('ALFRESCO_ROLE_EDITOR',       'Editor');        // Includes the Consumer permission group and adds Write and CheckOut
define('ALFRESCO_ROLE_CONSUMER',     'Consumer');      // Includes Read

// Define constants for the Alfresco capability names.
define('ALFRESCO_CAPABILITY_ALLOWED', 'ALLOWED');
define('ALFRESCO_CAPABILITY_DENIED',  'DENIED');
 ...
 */
    function is_configured() {
        ...
        return (!empty($CFG->repository_alfresco_server_host) &&
                !empty($CFG->repository_alfresco_server_port) &&
                !empty($CFG->repository_alfresco_server_username) &&
                !empty($CFG->repository_alfresco_server_password));
    }
...
*/
        ...
        if (empty($CFG->repository_alfresco_root_folder) ||
            ...
            if (empty($muuid)) {
                debugging(get_string('invalidpath', 'repository_alfresco'));
                return false;
            }
        ...
        // Otherwise, use the folder that the plug-in has been configured with.
            ...
            if (!$uuid = alfresco_uuid_from_path($CFG->repository_alfresco_root_folder)) {
                debugging(get_string('invalidpath', 'repository_alfresco'));
                return false;
            }
 ...
 */
    function process_config($config) {
        if (!isset($config->repository_alfresco_server_host)) {
            $config->repository_alfresco_server_host = '';
        }
        if (!isset($config->repository_alfresco_server_port)) {
            $config->repository_alfresco_server_password = '';
        }
        if (!isset($config->repository_alfresco_server_username)) {
            $config->repository_alfresco_server_password = '';
        }
        if (!isset($config->repository_alfresco_server_password)) {
            $config->repository_alfresco_server_password = '';
        }

        set_config('repository_alfresco_server_host', stripslashes(trim($config->repository_alfresco_server_host)));
        set_config('repository_alfresco_server_port', stripslashes(trim($config->repository_alfresco_server_port)));
        set_config('repository_alfresco_server_username', stripslashes(trim($config->repository_alfresco_server_username)));
        set_config('repository_alfresco_server_password', stripslashes(trim($config->repository_alfresco_server_password)));
 ...
 */
    function get_repourl() {
        ...
        if (ALFRESCO_DEBUG_TRACE) mtrace('get_repourl()');
        ...
        return $repourl . ':' . $CFG->repository_alfresco_server_port . '/alfresco/s/api';
    }
 ...
 */
    function get_webapp_url($gotologin = true) {
        ...
        if (ALFRESCO_DEBUG_TRACE) mtrace('get_webapp_url()');

        return $CFG->repository_alfresco_server_host . ':' . $CFG->repository_alfresco_server_port . '/alfresco/' .
 ...
 */
    function get_webdav_url() {
        ...
        if (ALFRESCO_DEBUG_TRACE) mtrace('get_webdav_url()');

        return $CFG->repository_alfresco_server_host . ':' . $CFG->repository_alfresco_server_port . '/alfresco/webdav/';
    }
 ...
 */
    function connect($username, $password) {
        ...
        if (ALFRESCO_DEBUG_TRACE) mtrace('connect(' . $username . ', ' . $password . ')');

        $this->errormsg = '';

        if (empty($username) || empty($password)) {
            $this->errormsg = get_string('usernameorpasswordempty', 'repository_alfresco');
            return false;
        }
 ...
 */
    function search($query, $catfilter = array()) {
        ...
        if (ALFRESCO_DEBUG_TRACE) mtrace('search(' . $query . ')');

        $this->errormsg = '';

        if (!empty($catfilter)) {
            $cats = get_records_list('alfresco_categories', 'id', implode(', ', $catfilter));
        }

        if (!empty($cats) && $query == '*') {
            $results = alfresco_category_search($cats);
        } else {
            $results = alfresco_search($query);
        }
 ...
 */
    function read_file($uuid, $localfile = '', $return = false, $process = true) {
        ...
        if (ALFRESCO_DEBUG_TRACE) mtrace('read_file(' . $uuid . ')');
...
//        }
    ...
    /// Check for a non-text file type to just pass through to the end user.
        ...
        $url = str_replace($node->filename, urlencode($node->filename), $node->fileurl) . '?alf_ticket=' .
               alfresco_utils_get_ticket('refresh', $username);
    ...
    /// Don't process non-text and "known" filetypes for relative links.
        if ((strpos($mimetype, 'text') !== 0 && !in_array($mimetype, $processtypes)) || !$process) {
            ...
            /// Cache this file for the required amount of time.
                if (!empty($CFG->repository_alfresco_cachetime)) {
                    $age = $CFG->repository_alfresco_cachetime;
                } else {
 ...
 */
    function fix_links(&$body, $path) {
        ...
        if (ALFRESCO_DEBUG_TRACE) mtrace('fix_links($body, ' . $path . ')');
 ...
 */
    function get_dirsize($uuid) {
        ...
        if (ALFRESCO_DEBUG_TRACE) mtrace('get_dirsize(' . $uuid . ')');
 ...
 */
    function download_dir($path, $uuid, $recurse = true) {
        ...
        if (ALFRESCO_DEBUG_TRACE) mtrace('download_dir(' . $path . ', ' . $uuid . ', ' . $recurse . ')');
        if (ALFRESCO_DEBUG_TIME) $start = microtime(true);
 ...
 */
    function copy_local($uuid, $filename, $location = '/') {
        if (ALFRESCO_DEBUG_TRACE) mtrace('copy_local(' . $uuid . ', ' . $filename . ', ' . $location . ')');
        ...
        if (!$this->read_file($uuid, $location . $filename)) {
            $this->errormsg = get_string('couldnotwritelocalfile', 'repository_alfresco');
            return false;
        }
 ...
 */
    function dir_exists($name, $uuid) {
        if (ALFRESCO_DEBUG_TRACE) mtrace('dir_exists(' . $name . ', ' . $uuid . ')');
 ...
 */
    function create_dir($name, $uuid = '', $description = '', $useadmin = true) {
        if (ALFRESCO_DEBUG_TRACE) mtrace('create_dir(' . $name . ', ' . $uuid . ', ' . $description . ', ' .
                                         ($useadmin ? 'true' : 'false') . ')');
        if (ALFRESCO_DEBUG_TIME) $start = microtime(true);
 ...
 */
    function get_course_store($cid, $create = true) {
        if (ALFRESCO_DEBUG_TRACE) mtrace('get_course_store(' . $cid . ')');
        ...
        if (!empty($uuid)) {
            ...
            if (!record_exists('alfresco_course_store', 'courseid', $cid)) {
 ...
 */
    function get_organization_store($oid, $create = true) {
        if (ALFRESCO_DEBUG_TRACE) mtrace('get_organization_store(' . $oid . ')');
        ...
        if (!empty($uuid)) {
            // Store the UUID value if it hasn't been stored already.
            if (!record_exists('alfresco_organization_store', 'organizationid', $oid)) {
                $organizationstore = new stdClass;
                $organizationstore->organizationid = $oid;
                $organizationstore->uuid           = $uuid;
                $organizationstore->id             = insert_record('alfresco_organization_store', $organizationstore);
            }
 ...
 */
    function get_user_store($uid, $nomigrate = false) {
        if (ALFRESCO_DEBUG_TRACE) mtrace('get_user_store(' . $uid . ')');
        ...
        if ($nomigrate) {
            ...
            if ($node = alfresco_create_dir($user->id, $this->uuuid, fullname($user) . ' (' . $user->email . ')')) {
                return $node->uuid;
            }
        } else {
            if (!($username = get_field('user', 'username', 'id', $uid))) {
                return false;
            }
            ...
            if (empty($this->uuuid)) {
                $this->uuuid = $this->alfresco_userdir($username);
            }
            return $this->uuuid;
        }
 ...
 */
    function is_company_home($uuid) {
        if (ALFRESCO_DEBUG_TRACE) mtrace('is_company_home(' . $uuid . ')');
 ...
 */
    function get_file_path($uuid) {
        if (ALFRESCO_DEBUG_TRACE) mtrace('get_file_path(' . $uuid . ')');
 ...
 */
    function permission_check($uuid, $uid = 0, $useurl = true) {
        ...
        if (ALFRESCO_DEBUG_TRACE) mtrace('permission_check(' . $uuid . ', ' . $uid . ', ' .
        ...
        if (!empty($CFG->repository_alfresco_root_folder)) {
            $moodleroot = $CFG->repository_alfresco_root_folder;
        } else {
            $moodleroot = '/moodle';
        }
 ...
 */
    function category_get($id = 0, $uuid = '') {
        if (ALFRESCO_DEBUG_TRACE) mtrace('category_get(' . $id . ', ' . $uuid . ')');

        if (!empty($id)) {
            if ($cat = get_record('alfresco_categories', 'id', $id)) {
                return $cat;
            }
        }

        if (!empty($uuid)) {
            if ($cat = get_record('alfresco_categories', 'uuid', $uuid)) {
                return $cat;
            }
        }
 ...
 */
    function category_get_children($catid) {
        if (ALFRESCO_DEBUG_TRACE) mtrace('category_get_children(' . $catid . ')');

        $cats = array();

        if ($children = get_records('alfresco_categories', 'parent', $catid)) {
 ...
 */
    function process_categories(&$uuids, $categories, $parent = 0) {
        if (ALFRESCO_DEBUG_TRACE) mtrace('process_categories(' . $uuids . ', ' . $categories . ', ' . $parent . ')');
        ...
        foreach ($categories as $category) {
            ...
            if (!$cat = $this->category_get(0, $category['uuid'])) {
                ...
                $cat->id     = insert_record('alfresco_categories', $cat);
            } else {
                $cat->parent = $parent;
                $cat->uuid   = $category['uuid'];
                $cat->path   = !empty($classification->category->id->path) ?
                ...
                update_record('alfresco_categories', $cat);
            }
...
*/
    function make_root_folder_select_tree($folders = false, $path = '') {
        ...
        if (ALFRESCO_DEBUG_TRACE) mtrace('make_root_folder_select_tree()');

        if (empty($folders) && (!$folders = alfresco_folder_structure())) {
            return false;
        }
        ...
        foreach ($folders as $i => $folder) {
            ...
            $text = ' <a href="#" onclick="set_value(\'' . $npath . '\')" title="' . $npath . '">' . $folder['name'] .
                    (!empty($CFG->repository_alfresco_root_folder) && $CFG->repository_alfresco_root_folder == $npath ?
 ...
 */
    function migrate_user($userorusername, $password = '') {
        ...
        if (ALFRESCO_DEBUG_TRACE) mtrace('migrate_user(' . (is_object($userorusername) ? 'object' : $userorusername) . ')');
 ...
 */
    function migrate_all_users($limit = 50) {
        ...
        if (ALFRESCO_DEBUG_TRACE) mtrace('migrate_all_users(' . $limit . ')');
        ...
        if (!empty($dir->folders)) {
            foreach ($dir->folders as $folder) {
                ...
                if (!empty($matches) && is_array($matches) && count($matches) === 1) {
                    ...
                    } else if (record_exists('user', 'id', $uid, 'deleted', 1)) {
 ...
 */
    function delete_user($username) {
        ...
        if (ALFRESCO_DEBUG_TRACE) mtrace('alfresco_delete(' . $username . ')');

        // Get the current value from the configuration option to enable or disable home directory deletion.
        $deletehomedir = !empty($CFG->repository_alfresco_deleteuserdir);

        return alfresco_delete_user($username, $deletehomedir);
    }
 ...
 */
    function allow_read($username, $uuid) {
        return alfresco_set_permission($username, $uuid, ALFRESCO_ROLE_CONSUMER, ALFRESCO_CAPABILITY_ALLOWED);
    }
 ...
 */
    function allow_edit($username, $uuid) {
        return alfresco_set_permission($username, $uuid, ALFRESCO_ROLE_COLLABORATOR, ALFRESCO_CAPABILITY_ALLOWED);
    }
 ...
 */
    function node_inherit($uuid, $inherit) {
        if (ALFRESCO_DEBUG_TRACE) mtrace('/moodle/nodeinherit/' . $uuid . '?enabled=' . ($inherit ? 'true' : 'false'));
 ...
 */
    function set_repository_location($uuid, $cid, $uid, $shared, $clear = false) {
        ...
        if ($clear) {
            unset($USER->alfresco_repository_location);
        }
 ...
 */
    function get_default_browsing_location(&$cid, &$uid, &$shared) {
        ...
        if (!isset($CFG->repository_alfresco_default_browse) ||
            $CFG->repository_alfresco_default_browse == ALFRESCO_BROWSE_MOODLE_FILES) {

            return '';

        // Or, handle determining if the user can actually access the chosen default location.
        } else if (isset($CFG->repository_alfresco_default_browse)) {
            if ($cid == SITEID) {
                $context = get_context_instance(CONTEXT_SYSTEM);
            } else {
                $context = get_context_instance(CONTEXT_COURSE, $cid);
            ...
            switch ($CFG->repository_alfresco_default_browse) {
                case ALFRESCO_BROWSE_ALFRESCO_SITE_FILES:
                ...
                case ALFRESCO_BROWSE_ALFRESCO_SHARED_FILES:
                    if (!empty($USER->access['rdef'])) {
                        foreach ($USER->access['rdef'] as $rdef) {
                            if (isset($rdef['block/repository:viewsharedcontent']) &&
                                      $rdef['block/repository:viewsharedcontent'] == CAP_ALLOW ||
                ...
                case ALFRESCO_BROWSE_ALFRESCO_COURSE_FILES:
                ...
                case ALFRESCO_BROWSE_ALFRESCO_USER_FILES:
                    if (empty($this->uuuid)) {
                        $this->uuuid = $this->alfresco_userdir($USER->username);
                    }
 ...
 */
    ...
    /// See if it's time to execute the cron task.
        if (!empty($CFG->repository_alfresco_cron) &&
            ((time() - $CFG->repository_alfresco_cron) < ALFRESCO_CRON_VALUE)) {

            return;
        }

    /// Execute the cron task.
        set_config('repository_alfresco_cron', time());

        $this->log = '';

        $this->log .= '   ' . get_string('startingalfrescocron', 'repository_alfresco');

        $categories = alfresco_get_categories();

        $this->log .= get_string('done', 'repository_alfresco') . "\n";
        $this->log .= '   ' . get_string('processingcategories', 'repository_alfresco');

        $uuids = array();
        $this->process_categories($uuids, $categories);

        if (!empty($uuids)) {
            delete_records_select('alfresco_categories', 'uuid NOT IN (\'' . implode('\', \'', $uuids) . '\')');
        }
        ...
        if (isset($this->uuuid)) {
            $this->log .= '   ' . get_string('startingpartialusermigration', 'repository_alfresco');
