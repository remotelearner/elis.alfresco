<?php

$string['alfrescocategoryfilter'] = 'Alfresco category filter';
$string['badqueryresponse'] = 'Bad query response!<br/>Your query has returned bad XML due to a Repository Server Error.';

$string['blockname'] = 'Alfresco Repository';
$string['categoryfilter'] = 'Category filter';

$string['details'] = 'Details';

$string['repository:createsitecontent'] = 'Create site-level content';
$string['repository:viewsitecontent'] = 'View site-level content';
$string['repository:createsharedcontent'] = 'Create shared content';
$string['repository:viewsharedcontent'] = 'View shared content';
$string['repository:createcoursecontent'] = 'Create course-level content';
$string['repository:viewcoursecontent'] = 'View course-level content';
$string['repository:createowncontent'] = 'Create personal content';
$string['repository:viewowncontent'] = 'View personal content';
$string['repository:createorganizationcontent'] = 'Create shared organization content';
$string['repository:vieworganizationcontent'] = 'View shared organization content';

$string['alfresconotconfigured'] = 'Alfresco is not configured. You can enable and configure the plug-in here:<br />' .
                                   '<p align=\"center\"><a href=\"$a\">Manage repositories</a></p>';
$string['confighelplink'] = 'If not empty, this link will be displayed in the block to allow linking to custom help ' .
                            'files.';
$string['configwebdavclient'] = 'If not empty, this link will be displayed in the block to allow users to access a ' .
                                'custom web-based WebDAV client.';
$string['embeddedwebdavlink'] = 'To access Alfresco via an embedded WebDAV client, click <a href=\"$a\">this link</a>.';
$string['helpfileslink'] = '<a href=\"$a\">Alfresco documentation</a>';
$string['helplink'] = 'Link to help documents';
$string['incompletequeryresponse'] = 'Incomplete query response!<br/>Your query failed to complete successfully due to a Repository Server Error.';
$string['norepositorypluginsystem'] = 'Your system is missing the repository plug-in system. This block requires that ' .
                                      'to be present with the Alfresco plug-in enabled in order to work.';
$string['usernameinfo'] = 'Use the same username and password that you use to access Moodle to login to Alfresco.';
$string['usernametenantinfo'] = 'Your username on Alfresco is:<br /><p><center><b>$a</b></center></p><br />' .
                                'Use the same password that you use to login to Moodle with.';
$string['webappaccess'] = 'To access the Alfresco web application directly, please use <a href=\"$a\" target=\"' .
                          'alfrescowebapp\">this link</a>.';
$string['webdavclient'] = 'Link to embedded WebDAV client';

?>
